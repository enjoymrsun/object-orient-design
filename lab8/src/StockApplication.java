...
public class StockApplication {
   ...
   * Get all stock data.
   *
   * @param companySymbols stock symbol as a array
   * @return all data for all stock
   * @throws Exception when cannot get that stock data
   */
  public static AllData getAllData(String[] companySymbols) throws Exception {
    AllData ad = new AllData();

    for (String company : companySymbols) {
   ...
   *
   * @param companySymbol company code
   * @return stock data
   * @throws Exception when cannot get that stock data
   */
  private static StockData getOneStockDatas(String companySymbol) throws Exception {
    /*
    ...
    String companyCode = companySymbol;
    String urlLink = "https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol="
            + companyCode + "&apikey=7NYRGP7J40JTAY2W&datatype=csv&outputsize=full";
    ...
    StockData sd = new StockData(companyCode);
    String headline = sc.next();

    while (sc.hasNext() && index < 3000) {
      ...
      DailyData data = new DailyData(companyCode, date, open, high, low, close, volume);
